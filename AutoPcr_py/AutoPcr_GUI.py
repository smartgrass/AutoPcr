...
cfg.read(configPath)
isJJC = Boolean(cfg.get('MainSetting','isJJC')) # 返回1

#保存配置
def SavaConfig(AllValues):
	cfg.set('MainSetting', 'isJJC', str(AllValues[0])) # 即可以修改已有的配置，也可以添加新的配置文件
...
layout = [  [sg.Checkbox('竞技场',isJJC)],[sg.Checkbox('探索',True)],[sg.Checkbox('地下城',True)],
			[sg.Checkbox('购买mana',True)],[sg.Checkbox('送装备',True)],
			[sg.Button('StartAndRun'), sg.Button('Run'),sg.Button('Save'), ] ]
